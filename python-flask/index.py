...
def signin():
    ...
    got=mycursor.execute("SELECT username FROM member WHERE username=%s",(username,))
    result=mycursor.fetchone() #我只需要讀取一個紀錄
    mydb.commit()
    if got == 0:
...
def signup():
    ...
    if got!=0:
       return redirect("/error/?message=註冊失敗，該用戶已被註冊")    
            ...
            result = mycursor.execute("INSERT INTO member (name,username,password) VALUES (%s,%s,%s)",
